...
%               not writing it).
function [ gen_image ] = summaryImage3( props, num_clusters, n_summaryImages, result_data, fileList, source, source_type, ini, labels_text )

    labels = unique(labels_text);

    gen_image = {};
    for i = 1:length(labels)
        if(isempty(labels_text))
            gen_image{i} = uint8(ones(props(1)*sum(labels_text==labels(i)), props(2)*n_summaryImages, 3)*255);
        else
            gen_image{i} = uint8(ones(props(1)*sum(labels_text==labels(i)), props(2)*(n_summaryImages+1), 3)*255);
        end
    end
    
    countI = ones(1,length(labels));
    for i = 1:num_clusters
        this_lab_id = find(ismember(labels, labels_text(i)));
        ...
        for j = 1:last
            if(j > num_sum)
                ...
                x1 = ((countI(this_lab_id)-1)*props(1));
                x2 = (countI(this_lab_id)*props(1));
                ...
                gen_image{this_lab_id} = step(htxtins, gen_image{this_lab_id});
                ...
                x1 = ((countI(this_lab_id)-1)*props(1)+1);
                x2 = (countI(this_lab_id)*props(1));
                y1 = ((j-1)*props(2)+1);
                y2 = (j*props(2));
                gen_image{this_lab_id}( x1:x2, y1:y2, 1 ) = im(:,:,1);
                gen_image{this_lab_id}( x1:x2, y1:y2, 2 ) = im(:,:,2);
                gen_image{this_lab_id}( x1:x2, y1:y2, 3 ) = im(:,:,3);
            end
        end
        
        countI(this_lab_id) = countI(this_lab_id)+1;
