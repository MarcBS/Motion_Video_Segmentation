...
classesShort = {'T', 'S', 'M'};
classes = {'In Transit', 'Static', 'Moving Head'}; % In Transit, Static, Moving Camera
classesColour = ['r'; 'g'; 'b'];
elemsToShow = 10000;
numBars = 10;
...
featuresTypeLong = {'Colour', 'Optical Flow', 'Blurriness'};

% Preparation of the features row for printing the result
rowFeatures = '|  ';
rowFeaturesCell = {}; rowFeaturesCell{1} = ' ';
count = 2;
for i = 1:length(featuresType)
    for j = 1:featuresNum(i)
        rowFeaturesCell{count} = [featuresType{i} num2str(j)];
...
fid = fopen('Grauman_ChiSq_Distances.csv','wt');
